...
class Product extends \common\components\ARModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['deposit', 'fee'], 'number'],
            [['one_profit', 'is_trade', 'source', 'force_sell', 'currency', 'hot', 'type', 'on_sale', 'state'], 'integer'],
     ...
     * @return 字符串
     */
    public static function getProductTradeTime($productId)
    {
        $model = self::findOne($productId);
        $time = $model->trade_time;
        if (!$time) {
        ...
        if (empty($timeArr)) {
            ...
            foreach ($timeArr as $arr) {
                $string .= $arr['start'] . '-' . $arr['end'] . '&nbsp;&nbsp;';
            }
        }
        return $string;
    }  
