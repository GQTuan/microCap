...
class WxTemplate
{
    ...
    public function getWechatUser($code) {   //以snsapi_userinfo为scope发起的网页授权，是用来获取用户的基本信息的
        //第二步：通过code换取网页授权 
        $tokenUrl = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=' . wechatInfo()->appid . '&secret=' . wechatInfo()->appsecret . '&code=' . $code . '&grant_type=authorization_code';
    ...
    public function getAccessToken($wechatUrl = null)
    {
        $url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.wechatInfo($wechatUrl)->appid.'&secret='.wechatInfo($wechatUrl)->appsecret;
        $options = [
    ...
    public function getWxInfo($url, $token, $prepay_id) {
        ...
        $wxSha1 = sha1($wxOri);
        //正式账号获取
        $sharejson = ['wxOri'=>$wxOri, 'ticket'=>$wxticket, 'appid'=>wechatInfo()->appid, 'timestamp'=>$timestamp, 'noncestr'=>$wxnonceStr, 'signature'=>$wxSha1];
    ...
    protected function getSign($timestamp, $wxnonceStr, $prepay_id)
    {
        ...
        $signType = 'MD5';
        $sign = sprintf("appId=%s&nonceStr=%s&package=%s&signType=%s&timeStamp=%s&key=" . wechatInfo()->mchkey, $appId, $nonceStr, $package, $signType, $timeStamp);
    ...
    public function getWxConfig($url) {
        ...
        return ['wxOri'=>$wxOri, 'ticket'=>$wxticket, 'appid'=>wechatInfo()->appid, 'timestamp'=>$timestamp, 'noncestr'=>$wxnonceStr, 'signature'=>$wxSha1];
    }  
}
