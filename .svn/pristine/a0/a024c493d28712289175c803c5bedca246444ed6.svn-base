...
$(function () {
    ...
    function chartAddPlotLine(chart){
        var closeprice = $('.now-price').text();
        //alert(closeprice);
        setTimeout(function(){
            chart.yAxis.removePlotLine('plot-line');
            chart.yAxis.addPlotLine({dashStyle:'dash',value:closeprice,width:1,color: '#FF6939',id: 'plot-line'}); 
        },100)
    ...
    var getPrice = function(chart, count) {
        ...
        $.get('/price.json?' + Math.random(), function(newData) {
            ...
            if(!isEmpty(newData)){
                ...
                var nowProduct = $(".product-list>li.active").data('name'),
                ...
                $('.product-list>li').each(function(){
                    if($(this).hasClass("active")){
                        beforePrice = $(this).find('.price').html();
                    }
                    var close = $(this).find('.price').html();
                    var name = $(this).data('name');
                    var className = $('.product-list>li.active .price').attr("class");
                    if (close != '休市') {
                        if(newData[name] == undefined){return;}
                        //console.log(newData[name] - close);
                        if(newData[name] != close){
                            $(this).find('.price').removeClass('up');
                            $(this).find('.price').removeClass('down');
                            className =""
                        }
                            
                        if (newData[name] > close) {
                            className = "up"
                            $(this).find('.price').addClass('up');
                        } else if (newData[name] < close) {
                            className = "down"
                            $(this).find('.price').addClass('down');
                        }
                        $(this).find('.price').html(newData[name]);
                        if($(this).hasClass("active")){
                            $(".now-price").html(newData[name]).removeClass('up').removeClass('down').addClass(className);
                        }
                    }
                });
    ...
    var getPrice1 = function(chart, count) {
        if ($('.selectProcut>li .isTrade'+data.pid+'>span').html() == '休市') {
            return false;
        }
        length = chart.data.length;
