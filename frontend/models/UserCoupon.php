...
class UserCoupon extends \common\models\UserCoupon
{
    ...
    public static function getNumberType($pid = 0)
    {
    ...
    //这个用户该期货的体验卷总数
    public static function getCouponIdCount($coupon_id, $bool = false)
    {
        if ($bool) {
            $coupon = Coupon::find()->where(['amount' => $coupon_id])->one();
            if (!$coupon) {
                return false;
            }
            $coupon_id = $coupon->id;
    ...
    //扣除用户使用该期货的体验卷
    public static function deleteUserCoupon($coupon_id, $hand = 1, $product_id = 0)
    {
